  ...
  async execute(input: ExecuteRewardActionInputDto): Promise<RewardActionResponseDto> {
    ...
    if (actionType === 'ADD_TO_CART' || actionType === 'PURCHASE') {
      if (!product.hasAvailableStock(1)) {
        throw new Error(`Operación inválida: El producto '${product.name}' no cuenta con stock disponible.`);
      }
    }
    ...
    if (!pointsToAward) {
      throw new Error(`La acción '${actionType}' no es elegible para acumulación de puntos.`);
    }
